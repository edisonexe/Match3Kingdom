...
namespace GameStateMachine.States
{
    public class PlayerTurnState : IState, IDisposable
    {
        ...
        private bool IsBlankPosition(Vector2Int gridPosition) => 
            _grid.GetValue(gridPosition.x, gridPosition.y).TileConfig.TileKind == TileKind.Blank;

        private bool IsValidPosition(Vector2Int gridPositon) => 
            gridPositon.x >= 0 && gridPositon.x < _grid.Width && 
            gridPositon.y >= 0 && gridPositon.y < _grid.Height;
    }
}